 ...
 * */
int assignENV(char* command, EnvVar EnvVariables[255], int get_env_counter){
    
    static int enviromentCounter = 0;
    if (get_env_counter){
...
int get_env_count(EnvVar EnvVariables[255]){
    return assignENV(" ", EnvVariables, 1);
}
 ...
 * */
int print_var(char* token, int enviromentCounter, EnvVar EnvVariables[255]){
    ...
    //if enviroment variable is not defined
...
/**
 * add each command entered to the log
 * @param command_log       log of all commands entered before current command
 * @param commandName       name of the command entered
 * @param exitCode          code either predefined in predefined functions or the fork return status
...
void addCommand(Command command_log[255], char *commandName, int exitCode, int commandCounter){
    time_t callTime;            // variable for when user calls command
    Command temp;               // temp variable for commands (to be passed to log)

    time(&callTime);
    strcpy(temp.name, commandName);
    memcpy(&temp.time, localtime(&callTime), sizeof(struct tm));
    memcpy(&temp.code, &exitCode, sizeof(int));
...
int main(int argc, char** argv){
    ...
    int cmd_return_value = 0;   // command return values for logging
    ...
    while(strcmp(input, "exit") != 0){ 
        ...
        if (command[0] == '$'){ // assign EnvVar
            cmd_return_value = assignENV(command, EnvVariables, 0);

        } else if (strcmp(command, "print") == 0) { // print arguments
            char *token;
            int temp_return_var = 0;

            for (int i = 1; i < argument_counter; i++) {
                token = arguments[i];

                if (token[0] == '$') { // argument is EnvVar
                    temp_return_var = print_var(token, get_env_count(EnvVariables), EnvVariables);
                } else {
            ...
            cmd_return_value = change_theme_auto(arguments);
        ...
        // add command. time and return value to log 
        addCommand(command_log, arguments[0], cmd_return_value, commandCounter); 
        commandCounter++;
    }
    return 0;
}